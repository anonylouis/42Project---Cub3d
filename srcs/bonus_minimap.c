...
static	void	add_pixel_floor_wall(t_graph *graph, int i, int j)
{
	int	x_minimap;
	int	y_minimap;

	x_minimap = floor(((double)i / (MINIMAP_SIZE - 1) - 0.5)
			* MINIMAP_N_CASE + graph->game.player_x);
	y_minimap = floor(((double)j / (MINIMAP_SIZE - 1) - 0.5)
	...
	if (y_minimap < 0 || y_minimap >= graph->game.nb_line_map
		...
		add_pixel_img(graph->img,
			MINIMAP_X + i, MINIMAP_Y + j, graph->game.minimap_wall);
}
...
void	print_minimap(t_graph *graph)
{
	...
	while (++i < MINIMAP_SIZE)
	{
		...
		{
			if (i < MINIMAP_CONTOUR || i >= MINIMAP_SIZE - MINIMAP_CONTOUR
				|| j < MINIMAP_CONTOUR || j >= MINIMAP_SIZE - MINIMAP_CONTOUR)
				add_pixel_img(graph->img,
					MINIMAP_X + i, MINIMAP_Y + j, graph->game.minimap_contour);
			else if (i >= MINIMAP_SIZE / 2 - 2 && i <= MINIMAP_SIZE / 2 + 2
				&& j >= MINIMAP_SIZE / 2 - 2 && j <= MINIMAP_SIZE / 2 + 2)
				add_pixel_img(graph->img,
					MINIMAP_X + i, MINIMAP_Y + j, graph->game.minimap_player);
			else
				add_pixel_floor_wall(graph, i, j);
		}
	}
}
